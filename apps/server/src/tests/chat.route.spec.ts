...
test('Create chat is successful with finalizedEventId not null and pendingEventId null', async () => {
    ...
    await expect(caller.chat.createChat({finalizedEventId: event.id, pendingEventId: null})).toBeDefined(); //should return channel id    
})
...
test('Create chat is successful with pendingEventId not null and finalizedEventId null', async () => {
    ...
    await expect(caller.chat.createChat({finalizedEventId: null, pendingEventId: pEvent.id})).toBeDefined(); //should return channel id
})
...
test('Create chat is fails with pendingEventId null and finalizedEventId null', async () => {
    ...
    await expect(caller.chat.createChat({finalizedEventId: null, pendingEventId: null})).rejects.toThrow(); 
})

//get admin tests
test('getAdmin is successful when given a valid channelId', async () => {
    const user = await db.insert(users).values({
                    name: "Tester1",
                    email: TESTER_EMAIL,
                    password: "Tester-Password1!",
        ...
        const result = await caller.chat.getAdmin({channelId: channel.id});
        await expect(result.adminUserId).toEqual(user.id);
})


test('getAdmin fails when given an invalid channelId', async () => {
    const user = await db.insert(users).values({
                    name: "Tester1",
                    email: TESTER_EMAIL,
                    password: "Tester-Password1!",
        ...
        await expect(caller.chat.getAdmin({channelId: -1})).rejects.toThrow();
})