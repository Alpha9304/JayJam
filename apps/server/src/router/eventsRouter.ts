...
import { db } from "../db";
import { finalizedEvents, finalizedEventParticipants, pendingEventParticipants, pendingEvents, pendingLocationOptions, pendingTimeOptions, users, PendingEvent, locationVotes, timeVotes, channels, bannedUsers } from "../db/schema";
...
export const getFinalizedParticipants = publicProcedure
    ...
    export const banParticipant = protectedProcedure
            .meta({ openapi: { method: 'POST', path: '/ban' } })
            .input(z.object({eventId: z.number(), userId: z.number()}))
            .output(z.object({success: z.boolean()}))
            .mutation(async ({ input}) => {     
    ...
    export const getBannedParticipants = protectedProcedure
        .query(async () => {
