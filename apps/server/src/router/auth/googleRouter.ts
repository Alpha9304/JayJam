...
import { db } from "../../db";
import { finalizedEvents, googleTokens } from "../../db/schema";
...
export const googleRouter = router({
  url: publicProcedure // FIX: turn to protected
    ...
    .mutation(async ({ctx}) => {
      ...
      return {
        ...
        url: getGoogleUrl(String(ctx.session.userId))
      }
  ...
  revokeToken: protectedProcedure
    ...
    .query(async ({ ctx }) => {
      ...
      if (!ctx.session) {
        console.error("No session inside revokeToken");
        throw new TRPCError({ message: "No session inside revokeToken", code: "UNAUTHORIZED" });
      }

      try {
        const [dbRes] = await db
          .select({ token: googleTokens.refreshToken })
          .from(googleTokens)
          .where(eq(googleTokens.userId, ctx.session.userId))
        ...
        // Send revoke request
        const encoded = encodeURIComponent(dbRes.token);
        const res = await fetch(`https://oauth2.googleapis.com/revoke?token=${encoded}`, {
          method: "POST",
          headers: {
        ...
        if (!res.ok) {
          console.error("Something went wrong while revoking token: ", res.statusText)
          return {
            success: false,
            message: `Something went wrong while revoking token: ${res.statusText}`
          }
        }
        ...
        // something went wrong
        if (!deletedToken) {
          console.error("Something went wrong while deleting token from db")
          return {
            success: false,
            message: `Something went wrong while deleting token from db`
          }
        }

      } catch (err) {
        console.error("Error while revoking Google token: ", err);
        throw new TRPCError({ message: `Error while revoking Google token ${err}`, code: "INTERNAL_SERVER_ERROR" });
      }
