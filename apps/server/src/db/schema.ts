...
import { sqliteTable, integer, text, primaryKey, blob, check, index } from "drizzle-orm/sqlite-core";

export const allowedThemes = ["light", "dark"] 
export type User = InferSelectModel<typeof users>;
export type UserInsert = InferInsertModel<typeof users>; 
export type FinalizedEvent = InferSelectModel<typeof finalizedEvents>;
export type PendingEvent = InferSelectModel<typeof pendingEvents>;
export type Group = InferSelectModel<typeof groups>;
export type Class = InferSelectModel<typeof classes>;
export type ClassParticipant = InferSelectModel<typeof classParticipants>;
...
export const messageReactions = sqliteTable("message_reactions",{
	  ...
	  userId: integer("user_id")
		.notNull()
		.references(() => users.id, { onDelete: "cascade" }),
	  emoji: text("emoji", { length: 8 }).notNull(),
	},(table) => [
...
export const bannedUsers = sqliteTable("bannedUsers", {
	id: integer("id").primaryKey({autoIncrement: true}),
