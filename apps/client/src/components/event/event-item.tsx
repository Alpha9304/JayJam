...
import { useGetBannedParticipants } from "@/hooks/event/use-get-banned-participants-";
...
export function EventItem({
  ...
  const {bannedUsers} = useGetBannedParticipants();
  const { userBasicInfo } = useUserInfo();
  

  const isBanned = bannedUsers.some(
    (ban) => ban.eventId === id && ban.userId === userBasicInfo.id
  );

  console.log("User: " + userBasicInfo.id + " Event: " + id)
