...
const loginPageUrl: string = `${TEST_CONFIG.frontendUrl}/`

const tester_email: string = "test1@jhu.edu"
...
test.describe('Sign Up Page', () => {
  ...
  test('Ensure register email is case insensitive', async ({ page }) => {
    await page.getByTestId('input-full-name').fill('John Doe');
    await page.getByTestId('input-email').fill("test1@jhU.edu");
    await page.getByTestId('input-password').fill('Password1!');
    await page.getByTestId('input-confirm-password').fill('Password1!');
    await page.getByTestId('button-complete-registration').click();
  ...
  test('Ensure register email is case insensitive 2', async ({ page }) => {
    await page.getByTestId('input-full-name').fill('John Doe');
    await page.getByTestId('input-email').fill("TeSt1@JhU.EDu");
    await page.getByTestId('input-password').fill('Password1!');
    await page.getByTestId('input-confirm-password').fill('Password1!');
  ...
  test('should allow navigation to login page', async ({ page }) => {
    ...
    await page.waitForURL(loginPageUrl); // Adjust if login page is different
  });
