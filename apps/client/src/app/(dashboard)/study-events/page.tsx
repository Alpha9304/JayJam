...
export default function MyStudyEventsPage() {
  ...
  useEffect(() => { //sort pending events by registration deadline, with earlier ones firt
    if(pendingEvents) {
      pendingEvents.sort(function(a, b) {
        const a_date: Date = new Date(a.registrationDeadline);
        const b_date: Date = new Date(b.registrationDeadline);
  ...
  const filterEvent = (event: FinalizedEvent, query: string) => {
    return (
      event.isParticipant && (event.title.toLowerCase().includes(query.toLowerCase()) ||
  ...
  const filteredPendingEvents = pendingEvents?.filter((event) => {
    const isParticipant = event.isParticipant;
    if (pendingFilter === "all") return isParticipant;
    if (pendingFilter === "created")
      return (event.eventCreatorId === userBasicInfo?.id) && isParticipant;
    return isParticipant;
  });

  const filteredFinalizedEvents = finalizedEventsList.filter((event) => {
    const isParticipant = event.isParticipant;
    if (finalizedFilter === "all") return isParticipant;
